...
def get_wikitext103(nsamples, seed, seqlen, model, hf_token, eval_mode=False):
    ...
    if eval_mode:
        testdata = datasets.load_dataset('wikitext', 'wikitext-103-v1', split='test')
        testenc = tokenizer(" ".join(testdata['text']), return_tensors='pt')
        return testenc
    else:
        traindata = datasets.load_dataset('wikitext', 'wikitext-103-v1', split='train')
...
def get_pile_books3(nsamples, seed, seqlen, model, hf_token, eval_mode=False):
...
def get_loaders(
    ...
    if 'wikitext103' in name:
        return get_wikitext103(nsamples, seed, seqlen, model, hf_token, eval_mode)
    if 'pile' in name:
        return get_pile_books3(nsamples, seed, seqlen, model, hf_token, eval_mode)